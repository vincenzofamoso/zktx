...
function homeKeyboard(userId) {
  const keyboard = new InlineKeyboard().text("Shield", "flow:shield").text("Private send", "flow:send").row().text("PONS market trade", "flow:market").text("Withdraw", "flow:withdraw").row().text("How it works", "how").text("Fees + burn", "fees").row();
...
bot.command("status", async (ctx) => { const jobs = store.jobs.filter((job) => job.userId === ctx.from?.id).slice(-5).reverse(), status = await protocolStatus(); const readiness = !status ? "Protocol API unavailable" : [`Vault: ${status.contractsReady ? "ready" : "not ready"}`, `Relayer: ${status.relayerReady ? "ready" : "offline"}`, `PONS keeper: ${status.marketKeeperReady ? "ready" : "offline"}`].join(" · "); await ctx.reply(`<b>ZKTX status</b>\n${escape(readiness)}\n\n${jobs.length ? jobs.map((job) => `${escape(job.type)}: ${escape(job.status)}`).join("\n") : "No recent actions."}`, { parse_mode: "HTML" }); });
bot.command("how", async (ctx) => ctx.reply("<b>How ZKTX works</b>\n\n1. Shield the RH token you already own.\n2. Ownership becomes an encrypted private note.\n3. For a market trade, the ZKTX vault executes against existing PONS liquidity in 3–7 slices.\n4. PONS sees the vault—not your originating wallet.\n5. Your actual proceeds return as shielded notes.\n\nPONS swaps remain public; wallet ownership and private-note transfers are shielded.", { parse_mode: "HTML" }));
bot.command("fees", async (ctx) => ctx.reply("<b>Fees and flywheel</b>\n\nPONS market trades charge 1.5% total:\n• 1% automatically buys and burns ZKTX\n• 0.5% supports execution\n\nMore protocol usage → more automatic ZKTX buying → more ZKTX permanently removed from supply.", { parse_mode: "HTML" }));
...
bot.callbackQuery("how", async (ctx) => { await ctx.answerCallbackQuery(); await ctx.reply("Shield RH tokens into private notes. ZKTX can then execute a PONS market trade through its vault, split across 3–7 blocks, and return the proceeds to you as shielded notes. The public sees vault execution—not your wallet as the trader."); });
bot.callbackQuery("fees", async (ctx) => { await ctx.answerCallbackQuery(); await ctx.reply("Market fee: 1.5%. One percent buys and burns ZKTX; 0.5% supports execution."); });
...
app.post("/api/v1/jobs/:id/authorize", async (req, res) => {
  try {
    ...
    const note = job.status === "authorized_ready" ? "Continue to generate the proof and execute." : "Authorization is saved, but execution is currently unavailable because the relayer or PONS keeper is offline.";
    await bot.api.sendMessage(job.chatId, `✅ ${job.type === "market" ? "PONS market trade" : job.type} authorized by ${job.wallet.slice(0, 8)}…${job.wallet.slice(-6)}. ${note}`, { reply_markup: keyboard });
