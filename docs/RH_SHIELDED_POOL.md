...
`RhRoutingSwapAdapter.sol` is the reviewed aggregation boundary for multiple venue adapters. Routes are allowlisted per direction-specific token pair; additions and replacements wait for the configured activation delay, while emergency disables are immediate. The router rejects fee-on-transfer inputs, verifies exact balance deltas and adapter-reported output, clears approvals after every call, prevents reentrancy, and uses two-step ownership. A route delay reduces configuration risk but does not make a newly added venue adapter safe; each route remains part of the trusted execution surface.
