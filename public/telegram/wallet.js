...
function encodeAbiParameters(params2, values) {
  if (params2.length !== values.length)
    throw new AbiEncodingLengthMismatchError({
      expectedLength: params2.length,
...
function prepareParams({ params: params2, values }) {
  const preparedParams = [];
  for (let i = 0; i < params2.length; i++) {
    preparedParams.push(prepareParam({ param: params2[i], value: values[i] }));
  }
...
var action = params.get("action") || "wallet";
var jobId = params.get("job") || "";
var iterations = 6e5;
var utf8 = new TextEncoder();
var databaseName = "zktx-trusted-device-v1";
var encode = (bytes) => btoa(String.fromCharCode(...bytes));
var decode = (value) => Uint8Array.from(atob(value), (character) => character.charCodeAt(0));
...
async function encrypt(secret, passphrase) {
  ...
  const account = privateKeyToAccount(normalized), salt = crypto.getRandomValues(new Uint8Array(16)), iv = crypto.getRandomValues(new Uint8Array(12)), key = await derive(passphrase, { salt: encode(salt), kdfIterations: iterations });
...
async function ready(account) {
  ...
  status("Wallet unlocked on this trusted device.");
}
